...
Top-level orchestrator for `/altex-triaging`. Given a `task_id` (and optionally free-form developer context), drives a four-phase investigation across the Settlement Engine, Account, and Instrument REST APIs/DBs — with the two backend services' Loki logs queried on demand during the investigation loop (Phase 3) — then writes a diagnosis report to `runs/<task_id>/diagnosis-report-<UTC-timestamp>.md` and prompts the engineer to capture novel findings into the playbook.
...
- **Work from the repo root.** Run every command and spawn every sub-agent from the `institutional-altex-wrapper` repo root. All `runs/<task_id>/…` paths in this skill are relative to that root (never the skill directory); `runs/` is the git-ignored top-level dir there. See `./docs/agent-output-format.md`.
- Read `./docs/agent-output-format.md` (skill-internal — see Rule 4 for the path convention).
...
1. **Evidence Collection** — `./phases/1-evidence-collection.md`. Orchestrator runs the `transfer-discoverer` script to fetch `transfer/tasks` (live + historical) for the transfer record, reads its output to reconcile the matching group and pin the single failed part, then fans out two always-on collectors — the `account-discoverer` and `instrument-discoverer` scripts — plus a conditional `error-code-resolver` sub-agent when the failed phase's log carries an exchange error code. *(The `settlement-engine-log-digger` and `transfer-engine-log-digger` sub-agents are **SKIPPED — pending logging-framework maturity**; Phase 3 pulls Loki logs on demand instead.)*
2. **Playbook Lookup** — `./phases/2-playbook-lookup.md`. **SKIPPED (pending: playbook population).** The orchestrator skips playbook scoring and proceeds directly to Phase 3, minting hypothesis #1 from Phase-1 evidence. When re-enabled, scoring `playbook/index.toon` entries against Phase 1 evidence will *bias* hypothesis selection — the loop structure is unchanged.
3. **Investigation Loop** — `./phases/3-investigation-loop.md`. Up to `MAX_ITER = 5` iterations. The orchestrator formulates each hypothesis (#1 from Phase-1 evidence, #N from the prior iteration's findings) and spawns `altex-investigator` to test that one hypothesis, pulling Loki / altex-DB / code / web on demand under its own tool budgets. Exit on a proven hypothesis (`verdict == "proven"` AND `confidence >= 0.8`); otherwise stop at the cap or on hypothesis exhaustion and report inconclusive. Spawn-layer failure or an empty-`results` file aborts (§ Abort protocol); a valid `inconclusive` does not.
4. **Issue Reporting** — `./phases/4-issue-reporting.md`. Reached only on a Phase-3 `proven` or `inconclusive` verdict (aborts and Phase-1 clean-stops never get here). Write the report per `./templates/success-report-template.md`, print the chat summary, and — only when the run **proved a root cause** — prompt for a playbook entry. *(On an `inconclusive` verdict there is nothing confirmed to capture, so no prompt.)*
...
1. **Collector pattern.** Each Phase 1/3 capability is a self-contained collector — this system has no actual triage skills. Collectors come in two kinds:
   ...
   - **Sub-agents** (`error-code-resolver`, `altex-investigator`; the `settlement-engine-log-digger` / `transfer-engine-log-digger` diggers are **SKIPPED** — see Phase 1) — spawned by `subagent_type` with a bare JSON object (filled `./prompts/<name>.json`) as the prompt. The orchestrator does NOT read the agent files; each agent owns its own procedure (which DB/logs/queries to run and the result shape it returns).
