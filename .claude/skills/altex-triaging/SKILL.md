...
4. **Issue Reporting** — `./phases/4-issue-reporting.md`. Write the report per `./templates/success-report-template.md`, print the chat summary, and prompt for a playbook entry if Phase 2 found no match. *(Phase 2 is currently SKIPPED, so there is never a match — prompt whenever a verdict was reached.)*
