...
(one entry per investigation-loop iteration, in chronological order; a one-clause parenthetical on a disproven/sub-threshold iteration — what it ruled out / where it pointed next — is welcome)
...
**Failed part summary:** <orchestrator-written 1-2 sentence interpretation of the failed part + failed phase, grounded in the `collect_transfer_evidence` API response (which group matched, the failed-phase triple, what the error log field shows)>

### Part logs

- `txn_log`: <truncate to ≤500 chars if longer, with [...truncated] marker; a bulky embedded blob such as a recon log's `raw.data` may be summarised in a parenthetical rather than dumped. A field that is `null` is shown as `null`.>
- `recon_src_log`: <ditto>
...
<orchestrator reading of the `collect_account_evidence` rows (the active `account_product` + `account` objects per side), focused on fields that look anomalous (exchange_status / internal_status not normal, missing exchange_uid / api_key_name on an exchange-backed product)>

### Instrument context

<orchestrator reading of the `collect_instrument_evidence` row (the active instrument object), focused on fields that look anomalous (status inactive, unexpected asset_type, decimal_convention out of range, symbol unknown to the platform)>
...
- `runs/<task_id>/collect-transfer-evidence-<ts>.json` (Settlement Engine API responses — live + historical)
- `runs/<task_id>/collect-account-evidence-<ts>.json`
- `runs/<task_id>/collect-instrument-evidence-<ts>.json`
