...
- **`signature.transfer_method`** — the chain/rail value from `transfer_task_part.transfer_method` literally (`BTC`, `ERC20`, `TRC20`, `BEP20`, `Solana`, `TON`, `XRP`, `Silvergate SEN`, `Signature`, `SIGNET`, `BCDC-*` variants, …; ~64 distinct values, NULL for exchange-internal hops). **Not** a category enum — do not write `EXCHANGE_WITHDRAW`/`ONCHAIN_WITHDRAW`/etc. (those values do not exist). Use `"*"` when the issue is rail-agnostic; let `task_type` + `error_patterns` narrow the match.
...
2. Run `./scripts/rebuild-playbook-index.sh` from the repo root.
...
The orchestrator writes the draft to `playbook/<proposed-id>.md` and re-runs `rebuild-playbook-index.sh` — but does **not** commit. The engineer reviews and edits before staging.

## Regeneration script

`scripts/rebuild-playbook-index.sh` reads every `playbook/*.md` (excluding `README.md`), extracts the frontmatter via `awk`, and rewrites `playbook/index.toon` in place. The script is idempotent (running it on an unchanged tree produces an identical file) and deterministic (rows sorted alphabetically by `id`).

Exit codes:
- `0` — index rebuilt (or unchanged).
- `1` — environment error (missing tool, no write permission, `playbook/` not found).
- `2` — at least one entry has malformed frontmatter; the offending file paths are printed to stderr.

Dependencies: `bash`, `awk` (BSD or GNU), `sort`, `mktemp`. No `yq` requirement.
